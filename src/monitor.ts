...
export class JupiterMonitor {
    ...
    async start() {
        ...
        try {
            ...
            await this.telegram.sendAlert('Monitor starting up and watching for LOGOS DCA orders...');
    ...
    private async pollDcaPositions() {
        ...
        while (this.isRunning) {
            try {
                ...
                // Filter for any positions involving LOGOS (either input or output)
                ...
                const currentPositions = new Map(
                    logosPositions.map(pos => [pos.publicKey.toString(), pos])
                );

                // Check for closed positions
                for (const [positionKey, oldPosition] of lastKnownPositions) {
                    if (!currentPositions.has(positionKey)) {
                        console.log('LOGOS DCA position closed:', positionKey);
                        
                        const wasBuyingLogos = oldPosition.account.outputMint.equals(this.LOGOS);
                        ...
                        const message = [
                            `${directionArrow} LOGOS DCA Position Closed`,
                            `Direction: ${wasBuyingLogos ? `${inputTokenInfo.symbol} ➜ LOGOS` : `LOGOS ➜ ${outputTokenInfo.symbol}`}`,
                ...
                for (const [positionKey, pos] of currentPositions) {
                    if (!lastKnownPositions.has(positionKey)) {
                        ...
                        const isBuyingLogos = pos.account.outputMint.equals(this.LOGOS);
                        ...
                        const directionArrow = isBuyingLogos ? '🟢 ⬆️' : '🔴 ⬇️';
                        
                        const message = [
                            `${directionArrow} LOGOS DCA Position (${isBuyingLogos ? 'Buying' : 'Selling'} LOGOS)`,
                            `Input Token: ${inputTokenInfo.symbol}`,
                            `Output Token: ${outputTokenInfo.symbol}`,
                            `Total Amount: ${tokenAmount} ${isBuyingLogos ? inputTokenInfo.symbol : 'LOGOS'}`,
                            `Amount Per Cycle: ${amountPerCycle} ${isBuyingLogos ? inputTokenInfo.symbol : 'LOGOS'}`,
